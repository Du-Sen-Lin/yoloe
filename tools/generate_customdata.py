...
base = '/root/dataset/glass_data_20250317'
duplicates = 0
...
for split in ['train']:
    ...
    coco = COCO(f'{base}/annotations/custom_{split}_segm.json')
                ...
                for a in coco.loadAnns(annIds):
                    ...
                    if a.get("segmentation") is not None and len(a["segmentation"]) > 0:
                        ...
                        if s not in segments:
                            ...
                            duplicates += 1
                    else: # 没有 segmentation 的话走 bbox 分支
                        assert(False)
...
coco = COCO(f'{base}/annotations/custom_train.json')
data = dict()
...
yaml_save('ultralytics/cfg/datasets/custom.yaml', data)